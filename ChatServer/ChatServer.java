...
public class ChatServer extends JFrame implements ActionListener {
    ...
    int port = 12345;
    StringTokenizer st;
    boolean pw = false; //준영 : 비밀번호 설정 유무 [0 = 비밀번호 없음 / X, 1 = 비밀번호 있음]
    ...
    class UserInfo extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    recv_Msg(msg + "/" + msg + "/" + msg + "/" + msg); // 준영 : 슬래시 안넣으면 에러남
        ...
        public void recv_Msg(String str) {
            ...
            if (protocol.equals("Note")) {
                String note = protocol2;      // 쪽지 내용 제대로 보내는 코드
                ...
                //다이얼로그의 리턴이 OK로 받을 경우 조건문을 통해 비밀번호 유무를 확인하여 전송(서버 : recv_Msg / CreateRoom)
                if (create_room_ok) // 방을 만들 수 있을 때
                {
                    ...
                    } else {             //준영 : 비밀번호가 설정된 경우 Message == RoomName, Message2 == 비밀번호 , 생성시에 room에 가입
                        RoomInfo r = new RoomInfo(Message, Message2, this);
                        room_vc.add(r); // 방을 추가
                        roomID = Message;
                        roomPW = Message2;
                        send_Msg("CreateRoom/" + Message);
                ...
                for (int i = 0; i < room_vc.size(); i++) {
                    ...
                    if (r.Room_name.equals(Message)) // 해당 방을 찾으면
                    {
                        if (r.Room_pw == "") { // 준영 : 비밀번호가 없으면 바로 입장
                            ...
                            String str1 = "";   // 동학 : 현재 참여자에 대한 String
                            for (int j = 0; j < r.Room_user.size(); j++) {   //   동학 : 참여자를 str1에 삽입
                                str1 = str1 + r.Room_user.get(j);
                                if (j != (r.Room_user.size() - 1)) {
                                    str1 = str1 + ", ";
                                }
                            }
                            // 동학 : 새로 참여한 사람에게 참여한 채팅방의 참여자 알려주는 메시지
                            send_Msg("Join_Room_B/참여자/" + str1);
            ...
            } else if (protocol.equals("ExitRoom"))   // 영원: 방나가기 프로토콜 메시지 받음
            {
                ...
                for (int i = 0; i < room_vc.size(); i++) {
                    ...
                    if (r.Room_name.equals(roomID)) // 해당 방을 찾으면
                    {
                        ...
                        r.Room_user.remove(userID);
                        r.Room_user_vc.remove(this);    // 채팅방에서 탈퇴
                        if (r.Room_user_vc.size() == 0) { // 영원 : 채팅방에 사람이 없으면 방나가기 메시지 보냄
                            room_vc.remove(r);
                ...
                for (int i = 0; i < room_vc.size(); i++) {
                    ...
                    if (r.Room_name.equals(Message2)) // 해당 방을 찾으면
                    {
                        if (r.Room_pw.equals(Message)) {//준영 : 비밀번호 체크
                            ...
                            send_Msg("Join_Room_B/현재 참여자/" + str1);   // 동학 : 새로 참여한 사람에게 참여한 채팅방의 참여자 알려주는 메시지
                            System.out.println(r.Room_user_vc);
                            r.Room_user_vc.add(this);    // 채팅방 가입, 사용자 추가
                            System.out.println(r.Room_user_vc.size());
                            roomID = Message2;
    ...
    class RoomInfo {
        ...
        private Vector<String> Room_user = new Vector<String>();   // 동학 : 방의 참여자를 저장하는 벡터

        RoomInfo(String name/* , String user */, UserInfo u) {            //준영 : 비밀번호가 없을 경우
            this.Room_name = name;
            System.out.println("RoomInfo u.userID : " + u.userID);
            this.Room_user_vc.add(u);
            System.out.println("비밀번호 설정안됨");
        }

        RoomInfo(String name/*, String user*/, String pw, UserInfo u) {   //준영 : 비밀번호가 들어왔을 경우
            this.Room_name = name;
            this.Room_pw = pw;
            this.Room_user_vc.add(u);
        ...
        public void BroadCast_Room(String str) {
            for (int i = 0; i < Room_user_vc.size(); i++) {
                ...
                if (!Room_user.contains(u.userID)) {   // 동학 : 벡터에 동일한 아이디가 있으면 입력하지 않는다.
                    this.Room_user.add(u.userID);   // 동학 : 방에 입장한 유저의 정보 저장
                }
                u.send_Msg(str);
            }
        }
