...
public class ChatClient extends JFrame implements ActionListener, KeyListener, ListSelectionListener {
    ...
    private void start() {
        ...
        Room_List.addMouseListener(new MouseAdapter() { // 영원: 새로 입장/입장된 방을 제외한 채팅방 선택 시 채팅방 참여 버튼 활성화
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == login_btn) {
            ...
            String room = Room_List.getSelectedValue();
            // 리스트에서 선택된 항목
            if (!My_Room.equals("")) {
            ...
            //방만들기 버튼 클릭 시 표시되는 다이얼로그의 오브젝트 지정
            Object[] inputFields = {
                    "방 이름 : ", room_name,
                    "비공개", pw_check,
                    "비밀번호 : ", room_pw};
            ...
            if (option == JOptionPane.OK_OPTION) {
                if (!str_RoomName.trim().equals("")) {
                    if (pw_check.isSelected()) {
                        if (!room_pw.getText().trim().equals("")) {
                            send_Msg("CreateRoom/" + str_RoomName + "/PassWord/" + room_pw.getText());
                        } else {
                            JOptionPane.showMessageDialog(null, "비밀번호가 공백입니다!", "알림", JOptionPane.ERROR_MESSAGE);
                        }
                    } else {
                        send_Msg("CreateRoom/" + str_RoomName + "/./.");
                    }
                } else {
                    JOptionPane.showMessageDialog(null, "방 이름이 공백입니다!", "알림", JOptionPane.ERROR_MESSAGE);
                }
            }
    ...
    private void sendMessage() {
        ...
        String time = fm.format(now);   //현재 시간을 fm형태로 포맷
    ...
    private void clientLogin() {
        ...
        } else if (id_tf.getText().trim().length() == 0) {
    ...
    private void recv_Msg(String str) {
        ...
        if (protocol.equals("NewUser")) {
            ...
            if (result == JOptionPane.YES_OPTION) { // 영원: 해당 방으로 join
                send_Msg("Join_Room/" + Message + "/." + "/.");
            }

        } else if (protocol.equals("CreateRoomFail")) // 방 만드는 것을 실패 
        {
            JOptionPane.showMessageDialog(null, "방만들기 실패", "알림", JOptionPane.ERROR_MESSAGE);

        } else if (protocol.equals("CreateRoom"))  // 서버가 CreateRoom, New_Room 동시에서 전송
        {
            joinroom_btn.setEnabled(false);
            create_room_btn.setEnabled(false);
            exitRoom_btn.setEnabled(true);
            inviteRoom_btn.setEnabled(true);
            msg_tf.setEditable(true);
            send_btn.setEnabled(true);
            My_Room = Message;
            chat_area.append(id + "님이 " + My_Room + "을 생성 및 가입하였습니다.\n");
            chat_area.setCaretPosition(chat_area.getDocument().getLength());   // 동학 : 글 작성시 자동으로 아래로 내려줌
        ...
        } else if (protocol.equals("Old_Room")) {
            joinroom_btn.setEnabled(true);
            roomListVC.add(Message);
            Room_List.setListData(roomListVC);
        ...
        } else if (protocol.equals("Join_Room_B")) // 7500
        {
            ...
            chat_area.setCaretPosition(chat_area.getDocument().getLength());   // 동학 : 글 작성 시 자동으로 아래로 내려줌
        } else if (protocol.equals("User_out")) {
            user_list.remove(Message);
            System.out.println(id + "\t" + Message);
            // 탈퇴자 정보 display
            chat_area.append(Message + "님 탈퇴 \n");
            // 동학 : 글 작성시 자동으로 아래로 내려줌
        ...
        } else if (protocol.equals("ExitRoom")) // 영원: 방나가기 마무리
        {
            ...
            System.out.println("protocal : Server_Out");
            stopped = true;
            try {
                socket.close();
            ...
            Room_List.setListData(roomListVC);
        } else if (protocol.equals("RequestPW")) { //준영 : 패스워드 요청
    ...
    public void keyReleased(KeyEvent e) {
        ...
        if (e.getSource() == id_tf) { // 동훈 : ID 입력창에서 엔터 입력 시 클라이언트 로그인 과정 진행
            if (key == KeyEvent.VK_ENTER) {
                clientLogin();
            }
        } else if (e.getSource() == msg_tf) { // 동훈 : 채팅 메시지필드에 글자 입력 후 엔터 입력 시 메시지 전송 과정 진행
